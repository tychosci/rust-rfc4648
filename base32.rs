...
fn mk() -> base32 {
    ...
    let table = vec::to_mut(vec::from_elem(32u, 0u8)), i = 0u8;
    u8::range(65u8, 91u8)  { |j| table[i] = j; i += 1u8; }
    u8::range(50u8, 56u8)  { |j| table[i] = j; i += 1u8; }

    let table_hex = vec::to_mut(vec::from_elem(32u, 0u8)), i = 0u8;
...
fn b32encode(table: [u8], src: [u8]) -> [u8] {
    ...
    let input = vec::to_mut(vec::from_elem(5u, 0u8));
    let output = vec::to_mut(vec::from_elem(8u, 0u8));
...
fn b32decode(src: [u8], b32idx: native fn(u8) -> u8) -> [u8] {
    ...
    let targ: [mutable u8] = [mutable];
    let input = vec::to_mut(vec::from_elem(8u, 0u8));
    let output = vec::to_mut(vec::from_elem(8u, 0u8));
    let curr = 0u, src_curr = 0u;
    let targlen =
